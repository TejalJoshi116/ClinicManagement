...
class UpdateClass:
    def __init__(self, row_id):
        ...
        if record:
            self.firstname = record[0][1]
            self.lastname = record[0][2]
            self.dateOfBirth = record[0][3]
            self.monthOfBirth = record[0][4]
            self.yearOfBirth = record[0][5]
            self.gender = record[0][6]
            self.address = record[0][7]
            self.contactNumber = record[0][8]
            self.bloodType = record[0][9]
            self.symptom = record[0][10]
            self.rubric = record[0][11]
            self.medicine = record[0][12]
            self.notes = record[0][13]
            self.amount = record[0][14]
        else:
...
# fg=self.fg_color, bg=self.bg_color,
    ...
    def updateData(self):
        ...
        dob = f"{self.dateOfBirthBox.get()}/{self.monthOfBirthBox.get()}/{self.yearOfBirthBox.get()}"
