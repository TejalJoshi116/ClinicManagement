...
class HomePage:
    ...
    def Update(self):
        ...
        self.updateIDWindow.title("Update Patient Information")
        self.updateIDWindow.geometry("400x200")
        self.updateIDWindow.config(bg='#F8F8F8')
        font = ("Arial", 14)
        ...
        # Label
        tkinter.Label(self.updateIDWindow, text="Enter the ID to update", font=font, bg='#F8F8F8').grid(pady=20, row=1)

        # Entry widgets
        self.idEntry = tkinter.Entry(self.updateIDWindow, width=5, textvariable=self.id, font=font)

        self.idEntry.grid(pady=10, row=2)

        # Button widgets
        tkinter.Button(self.updateIDWindow, text="Update", command=self.updateID, font=font, bg='#00796b', fg='#fff', borderwidth=0).grid(pady=10, row=3)
